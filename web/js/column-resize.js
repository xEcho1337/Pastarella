(function () {
    ...
    function readWidthMap(table, keep) {
        ...
        headerCells(table).forEach(function (th) {
            ...
            map[name] = Math.round(
                parseFloat(th.style.width) || th.offsetWidth,
            );
        });
        return map;
    }
    ...
    function openColumnMenu(table, page, index, x, y) {
        ...
        function refreshMenuState() {
            var hidden = loadHidden(page, index);
            var visible = names.filter(function (n) {
                return hidden.indexOf(n) === -1;
            });
            Array.prototype.forEach.call(
                menu.querySelectorAll(".col-menu-item"),
                function (item, i) {
        ...
        menu.style.top =
            Math.max(4, Math.min(y, window.innerHeight - rect.height - 4)) +
            "px";
        openMenu = menu;
    }
    ...
    function onHeaderDown(e, table, page, index, th) {
        ...
        function onMove(ev) {
            ...
            if (ref) ref.classList.add("drop-before");
                ...
                var cells = headerCells(table).filter(function (c) {
                    return c !== th;
                });
                if (cells.length)
                    cells[cells.length - 1].classList.add("drop-after");
            }
        }
