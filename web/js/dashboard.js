...
(function () {
    ...
    function renderStats(report) {
        ...
        setText(
            "statServicesSub",
            running + " running / " + report.Services.length + " total",
        );
    }
    ...
    function renderChart(report) {
        ...
        data.forEach(function (d) {
            ...
            bar.style.height =
                Math.max(3, Math.round((d.value / max) * 120)) + "px";
    ...
    function renderSummary(report) {
        var unsigned =
            report.Processes.filter(function (p) {
                return !p.Signer && !p.Sha256;
            }).length +
            report.Drivers.filter(function (d) {
                return !d.Signer;
            }).length;
        ...
        if (sub) {
            sub.textContent =
                "Forensic collection summary from the AnalysisReport · " +
                report.Processes.length +
                " processes, " +
                report.Persistences.length +
                " persistences.";
        }
    }
