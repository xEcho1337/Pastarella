...
(function () {
    ...
    var state =
        {}; /* tbodyId -> { rows, original, colNames, sortIdx, sortDir, page, perPage } */
    ...
    function cellSearchText(cell) {
        ...
        if (cell && cell.__element)
            return text + " " + titlesOf(cell.__element);
        if (cell && typeof cell === "object" && cell.nodeType === 1) {
    ...
    function matchesFilter(s, row) {
        ...
        if (s.filterCol >= 0 && s.filterCol < row.length) {
            return (
                cellSearchText(row[s.filterCol]).toLowerCase().indexOf(q) !== -1
            );
        }
        for (var i = 0; i < row.length; i++) {
            if (cellSearchText(row[i]).toLowerCase().indexOf(q) !== -1)
                return true;
        }
        return false;
    }
    ...
    function refreshRows(s) {
        s.rows = s.original.filter(function (row) {
            return matchesFilter(s, row);
        });
    ...
    function displayOrder(table, s) {
        ...
        var map = Array.prototype.map.call(
            table.querySelectorAll("thead th"),
            function (th) {
                return s.colNames.indexOf(headerName(th));
            },
        );
        if (
            map.some(function (i) {
    ...
    function render(tbodyId) {
        ...
        if (slice.length === 0) {
            ...
            td.colSpan = table
                ? Math.max(1, table.querySelectorAll("thead th").length)
                : 1;
            td.textContent = s.filterText
        ...
        if (
            ...
            window.PastarellaColumns.reapplyOrder
        ) {
