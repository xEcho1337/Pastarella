...
input.addEventListener("change", async (event) => {
	...
	data["Processes"].forEach(d => appendArrToTable(document.getElementById("processes"),
		[
			d["Id"], d["Name"], d["Path"],
			d["Sha256"], signerIcon(d["Signer"]), d["StartTime"],
			stringifyMetadata(d["Metadata"]),
		])
	);

	data["Services"].forEach(d => appendArrToTable(document.getElementById("services"),
		[
			d["Status"], d["ServiceType"], d["ServiceName"],
			d["DisplayName"], `${d["ExecPath"]} ${d["Arguments"].join(' ')}`, d["Sha256"],
		])
	);

	data["OpenPorts"].forEach(d => {
		let state = d["State"] != null ? d["State"] : "";
		let remote = d["Remote"] != null ? `${d["Remote"]["Ip"]}:${d["Remote"]["Port"]}` : "";

		appendArrToTable(document.getElementById("open-ports"),
			[
				d["Protocol"], state,
				`${d["Local"]["Ip"]}:${d["Local"]["Port"]}`, remote,
				d["ProcessId"], d["ProcessName"],
			])
	});

	data["Users"].forEach(d => appendArrToTable(document.getElementById("users"),
		[
			d["Name"], d["Description"], d["Uid"],
			d["Home"], booleanIcon(d["Disabled"]), stringifyMetadata(d["Metadata"]),
		])
	);

	data["Hosts"].forEach(d => appendArrToTable(document.getElementById("hosts"),
		[
			d["Ip"], d["Domain"],
		])
	);

	data["Drivers"].forEach(d => appendArrToTable(document.getElementById("drivers"),
		[
			d["Name"], d["DisplayName"], d["Identifier"],
			d["Type"], d["ExecutablePath"], d["Version"],
			booleanIcon(d["Loaded"]), d["Sha256"], signerIcon(d["Signer"]),
		])
	);
	...
	data["Persistences"].forEach(d => {
		...
		[
			d["RiskScore"], d["Name"], d["Path"], actionStr,
			d["Trigger"], d["Privilege"], d["Type"],
			stringifyMetadata(d["Metadata"]),
		])
	});

	data["Storages"].forEach(d => appendArrToTable(document.getElementById("storages"),
		[
			d["Type"], d["Name"],
	...
	Object.entries(data["Envs"]).forEach(([k, v]) => appendArrToTable(document.getElementById("envs"), [ k, v ]));
});
